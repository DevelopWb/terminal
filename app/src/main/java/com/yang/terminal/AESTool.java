...
public class AESTool {
    ...
    private static String pwd = "lyx123456ybf";
    ...
    public static String des(String content, @AESType int type) {
        if (TextUtils.isEmpty(content) || TextUtils.isEmpty(pwd)) {
            return null;
        }
        try {
            SecretKeySpec secretKeySpec;
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                secretKeySpec = deriveKeyInsecurely(pwd);
            } else {
                secretKeySpec = fixSmallVersion(pwd);
            }
